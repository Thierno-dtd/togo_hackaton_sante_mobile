...
class _MedicationTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final provider = context.watch<AppProvider>();
    final meds = provider.medicationReminders;
    final isDark = Theme.of(context).brightness == Brightness.dark;
                            ...
                            children: [
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text('Prises', style: AppTextStyles.caption.copyWith(color: AppColors.textSecondary)),
                                    const SizedBox(height: 4),
                                    Wrap(
                                      spacing: 6,
                                      children: med.intakeTimes.map((t) => Container(
                              ...
                              Column(
                                ...
                                children: [
                                  Text('Stock', style: AppTextStyles.caption.copyWith(color: AppColors.textSecondary)),
                                  const SizedBox(height: 4),
                                  Text('${med.stock} unités', style: AppTextStyles.body.copyWith(
                                    fontWeight: FontWeight.w700,
                                    color: med.needsRenewal ? AppColors.warning : AppColors.success,
                                  )),
                                  Text('Alerte à ${med.renewalAlertThreshold}', style: AppTextStyles.caption),
                                ],
                              ),
                            ],
                          ),
  ...
  void _showAddMedSheet(BuildContext ctx) {
    showModalBottomSheet(
      ...
      builder: (_) => _AddMedicationSheet(outerContext: ctx),
    );
  }
}
