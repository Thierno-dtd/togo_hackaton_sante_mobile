...
class _ChartsTab extends StatelessWidget {
  ...
  Widget _buildHypertensionCharts(
      BuildContext ctx, AppProvider prov, bool dark) {
    ...
    if (records.isEmpty) {
      return const EmptyState(
          ...
          subtitle:
              'Ajoutez votre première mesure pour voir les graphiques');
    }

    return ListView(
      padding: const EdgeInsets.all(20),
      children: [
        _buildHypertensionSummaryCards(ctx, prov, dark),
            ...
            children: [
              Row(children: [
                const Icon(Icons.favorite,
                    color: AppColors.hypertension, size: 18),
        ...
        // Mini charts — overflow-safe avec LayoutBuilder
            ...
            children: [
              SizedBox(
                  ...
                  child: _miniChart(
                      ...
                      'bpm',
                      AppColors.error,
                      records.map((r) => r.heartRate).toList(),
                      dark)),
              ...
              SizedBox(
                  ...
                  child: _miniChart(
                      ...
                      '°C',
                      AppColors.warning,
                      records.map((r) => r.temperature).toList(),
                      dark)),
            ],
          );
        }),
        ...
        AppCard(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 4),
              Text('${records.length} mesures enregistrées',
                  style: AppTextStyles.bodySmall),
              const SizedBox(height: 20),
              SizedBox(
                height: 220,
                child:
                    LineChart(_buildGlucoseChart(records, dark)),
              ),
            ],
          ),
        ),
  ...
  Widget _buildHypertensionSummaryCards(
      BuildContext ctx, AppProvider prov, bool dark) {
        ...
        children: [
          SizedBox(
            ...
            child: MetricCard(
              ...
              unit: 'mmHg',
              status: AppUtils.bpStatus(
                  latest.systolic, latest.diastolic),
              statusColor:
                  AppUtils.bpColor(latest.systolic, latest.diastolic),
              icon: Icons.favorite,
          ...
          SizedBox(
            ...
            child: MetricCard(
              ...
              statusColor:
                  AppUtils.heartRateColor(latest.heartRate),
        ...
        children: [
          SizedBox(
            ...
            child: MetricCard(
              ...
              statusColor:
                  AppUtils.glucoseColor(latest.glucoseLevel),
  ...
  LineChartData _buildBpChart(
      List<HypertensionRecord> records, bool dark) {
    final spots1 = records
        .asMap()
        .entries
        .map((e) => FlSpot(e.key.toDouble(), e.value.systolic))
        .toList();
    final spots2 = records
        .asMap()
        .entries
          ...
          getTitlesWidget: (v, _) {
            ...
            if (idx < records.length) {
              return Padding(
                ...
                child: Text(
                    AppUtils.formatDateShort(
                        records[idx].measuredAt),
                    style: AppTextStyles.caption),
              );
            }
      ...
      lineBarsData: [
        LineChartBarData(
          ...
          belowBarData: BarAreaData(
              show: true,
              color: AppColors.hypertension.withOpacity(0.08)),
        ),
        LineChartBarData(
          spots: spots2,
  ...
  LineChartData _buildGlucoseChart(
      List<DiabetesRecord> records, bool dark) {
    final spots = records
        .asMap()
        .entries
        .map((e) =>
  ...
  Widget _miniChart(BuildContext ctx, String title, String unit,
      Color color, List<double> values, bool dark) {
    final spots = values
        .asMap()
        .entries
...
class _HistoryTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (isHypertension) {
      ...
      return ListView.separated(
        ...
        itemBuilder: (_, i) =>
            _buildHypertensionItem(context, records[i], isDark),
      );
    } else {
      ...
      return ListView.separated(
        ...
        separatorBuilder: (_, __) => const SizedBox(height: 10),
        itemBuilder: (_, i) =>
            _buildDiabetesItem(context, records[i], isDark),
      );
    }
  }

  Widget _buildHypertensionItem(
      BuildContext context, HypertensionRecord r, bool dark) {
    ...
    return AppCard(
      ...
      child: Column(
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                        '${r.systolic.toInt()}/${r.diastolic.toInt()} mmHg',
                        style: AppTextStyles.h3.copyWith(
                            color: dark
                                ? AppColors.darkText
                                : AppColors.textPrimary)),
              ...
              Column(
                ...
                children: [
                  Text(AppUtils.formatDate(r.measuredAt),
                      style: AppTextStyles.caption),
                  Text(AppUtils.formatTimeFromDateTime(r.measuredAt),
                      style: AppTextStyles.caption),
                ],
              ),
            ],
          ...
          const AppDivider(),
          const SizedBox(height: 12),
          // ─── Overflow-safe mini metrics ───
          LayoutBuilder(builder: (context, constraints) {
            return Wrap(
              spacing: 8,
              runSpacing: 8,
              children: [
                _miniMetric(Icons.monitor_heart,
                    '${r.heartRate.toInt()} bpm',
                    AppUtils.heartRateColor(r.heartRate)),
                _miniMetric(Icons.thermostat,
                    '${r.temperature.toStringAsFixed(1)} °C',
                    AppUtils.temperatureColor(r.temperature)),
                if (r.context.isNotEmpty)
                  _miniMetric(Icons.info_outline,
                      _contextLabel(r.context), AppColors.textHint),
              ],
            );
          }),
  ...
  Widget _buildDiabetesItem(
      BuildContext context, DiabetesRecord r, bool dark) {
          ...
          Row(
            children: [
              Container(
                ...
                height: 44,
                decoration: BoxDecoration(
                    color: glColor.withOpacity(0.12),
                    borderRadius: BorderRadius.circular(12)),
                child:
                    Icon(Icons.water_drop, color: glColor, size: 22),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                    ...
                    Text('${r.glucoseLevel.toStringAsFixed(2)} g/L',
                        style: AppTextStyles.h3.copyWith(
                            color: dark
                                ...
                                : AppColors.textPrimary)),
                    Text(
                        AppUtils.glucoseStatus(r.glucoseLevel,
                            fasting: r.context == 'a_jeun'),
                        style: AppTextStyles.bodySmall.copyWith(
                            color: glColor,
                            fontWeight: FontWeight.w600)),
                  ],
            ...
            return Wrap(
              ...
              children: [
                ...
                _miniMetric(Icons.thermostat,
                    ...
                    AppUtils.temperatureColor(r.temperature)),
                _miniMetric(Icons.info_outline,
                    _diabetesContextLabel(r.context),
                    AppColors.textHint),
              ],
            );
          }),
...
class _HypertensionMeasurementSheet extends StatefulWidget {
  ...
  @override
  State<_HypertensionMeasurementSheet> createState() =>
      _HypertensionMeasurementSheetState();
}

class _HypertensionMeasurementSheetState
    extends State<_HypertensionMeasurementSheet> {
  ...
  void _save() {
    if (_systolicCtrl.text.isEmpty || _diastolicCtrl.text.isEmpty) {
      AppUtils.showSnackBar(widget.outerContext,
          'Renseignez la tension artérielle',
          isError: true);
      return;
    }
    ...
    final temperature =
        double.tryParse(_temperatureCtrl.text) ?? 37.0;
    ...
    if (systolic < 60 ||
        ...
        diastolic > 150) {
      AppUtils.showSnackBar(widget.outerContext,
          'Valeurs de tension non valides',
          isError: true);
      return;
    }
    ...
    final record = HypertensionRecord(
      id: const Uuid().v4(),
      userId:
          widget.outerContext.read<AppProvider>().currentUser!.id,
      systolic: systolic,
      diastolic: diastolic,
      heartRate: heartRate,
            ...
            child: Row(
              children: [
                Container(
                  ...
                  height: 40,
                  decoration: BoxDecoration(
                      color: AppColors.hypertension.withOpacity(0.12),
                      borderRadius: BorderRadius.circular(12)),
                  child: const Icon(Icons.favorite,
                      color: AppColors.hypertension, size: 20),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('Tension artérielle & signes vitaux',
                          style: AppTextStyles.bodySmall),
                    ],
                  ),
                ),
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  _buildSectionLabel(
                      'Tension artérielle *', AppColors.hypertension),
                  const SizedBox(height: 10),
                  Row(
                    children: [
                      Expanded(
                  ...
                  if (_systolicCtrl.text.isNotEmpty &&
                      _diastolicCtrl.text.isNotEmpty)
                    _buildBpPreview(),
                  const SizedBox(height: 20),
                  ...
                  const SizedBox(height: 20),
                  _buildSectionLabel(
                      'Date et heure', AppColors.primary),
                  const SizedBox(height: 10),
                  Row(
                    children: [
                      Expanded(
                  ...
                  const SizedBox(height: 20),
                  _buildSectionLabel('Commentaire (optionnel)',
                      AppColors.textSecondary),
                  const SizedBox(height: 10),
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      hintText:
                          'Ex: après repos, prise de médicament...',
                      prefixIcon:
        ...
        child: Row(
          children: [
            const Icon(Icons.access_time,
                size: 16, color: AppColors.primary),
            const SizedBox(width: 8),
            Text(AppUtils.formatTime(_selectedTime),
                style: AppTextStyles.body),
          ],
        ),
      ),
...
class _DiabetesMeasurementSheet extends StatefulWidget {
  ...
  @override
  State<_DiabetesMeasurementSheet> createState() =>
      _DiabetesMeasurementSheetState();
}

class _DiabetesMeasurementSheetState
    extends State<_DiabetesMeasurementSheet> {
  ...
  void _save() {
    if (_glucoseCtrl.text.isEmpty) {
      AppUtils.showSnackBar(
          widget.outerContext, 'Renseignez la glycémie',
          isError: true);
      return;
    }
    ...
    if (glucose < 0.1 || glucose > 6.0) {
      AppUtils.showSnackBar(widget.outerContext,
          'Valeur de glycémie non valide (0.1 - 6.0 g/L)',
          isError: true);
      return;
    }
    final heartRate = double.tryParse(_heartRateCtrl.text) ?? 72;
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(
                          child: _datePicker(context, isDark)),
                      const SizedBox(width: 12),
                      Expanded(
                  ...
                  const SizedBox(height: 20),
                  _sectionLabel('Commentaire (optionnel)',
                      AppColors.textSecondary),
                  const SizedBox(height: 10),
                  TextField(
                    controller: _commentCtrl,
                    ...
                    decoration: const InputDecoration(
                      hintText:
                          'Ex: à jeun depuis 8h, repas copieux...',
                      prefixIcon:
                          Icon(Icons.comment_outlined, size: 20),
                    ),
                  ),
  ...
  Widget _glucosePreview() {
    ...
    final status =
        AppUtils.glucoseStatus(g, fasting: _context == 'a_jeun');
